########################
##
##  A Tutorial on Working with Dates and Times in R
##
##  Leilah Harouni
##
########################

# Load packages
library(tidyverse)
library(lubridate)


############# DATES TUTORIAL ################

# Lubridate cheatsheet:
# https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_lubridate.pdf


# When working with dates in R, there are "dates" and "date-times"

# A date example: June 21st, 1988
# A date-time example: 1988-06-21 06:22:30

# To format dates, lubridate provides a series of functions 
# that are a permutation of the letters "m", "d" and "y" 
# to represent the ordering of month, day and year 

mdy("August 28, 1996")
mdy("8-28-1996")
mdy("08/28/1996")
mdy("8/28/96")

dmy("28-August-1996")
dmy("28 August 1996")

ymd("1996 8 28")
ymd("1996-08-28")

bday <- mdy("August 28th, 1996")
bday

# To check what kind of variable bday is:
class(bday)

# It prints like character data, but it actually represents the number of days since
# 1970-01-01. Dates prior to 1970-01-01 are negative numbers. 

# Use as.numeric() to verify.
as.numeric(bday)

# Lubridate can also handle date-times, even if they include AM/PM times
bday2 <- "August 28st, 1996 17:22:30"
mdy_hms(bday2)

bday3 <- "August 28st, 1996 5:22:30PM"
mdy_hms(bday3)

# You can also calculate durations between dates using %--%
today <- "October 26th, 2020"
today <- mdy(today) # change it from a character string to a date variable
my_age <- bday %--% today # put the earlier day first

as.numeric(my_age) # number of seconds between the two dates
as.duration(my_age) 

time_length(my_age, unit = "months") # to get a more interprable duration 
time_length(my_age, unit = "years")
time_length(my_age, unit = "days")

# You can also read in times without dates using the functions ms, hm, or hms,
# where again "h", "m", and "s" stand for "hours", "minutes", and "seconds"

# Examples:
time1 <- c("1:13", "0:58", "1:01")
ms(time1)

time2 <- c("12:23:11", "09:45:31", "12:05:22")
hms(time2)

time3 <- c("2:14", "2:16", "3:35")
hm(time3)

time1 <- ms(time1)
time2 <- hms(time2)
time3 <- hm(time3)

# But don't forget that these are not character strings! 
# These times are actually stored as seconds
as.numeric(time1)
as.numeric(time2)
as.numeric(time3)

## Exercise 1: When did Trump tweet the most in 2016?

# Read in the data
trump_tweets <- read_csv("https://raw.githubusercontent.com/mkearney/trumptweets/master/data/trumptweets-1515775693.tweets.csv")
head(trump_tweets)
str(trump_tweets)

# 1. "created_at" is our date-time variable; the first step is to treat it as such
trump_tweets <- trump_tweets %>%
  mutate(created_at = ymd_hms(created_at))

str(trump_tweets)
head(trump_tweets$created_at)
as.numeric(head(trump_tweets$created_at))

# 2. Create a new column called "date" that contains the date but ignores the time
trump_tweets <- trump_tweets %>%
  mutate(date = date(created_at))

# 3. Create a new column that stores just the month of the tweet 
# Make sure the month is labeled but abbreviated
trump_tweets <- trump_tweets %>%
  mutate(month = month(date, label = TRUE, abbr = TRUE)) #abbr = false will give it the whole month name 

# 4. Create a new column that stores just the year of the tweet
trump_tweets <- trump_tweets %>%
  mutate(year = year(date))

# 5. Use a pipe to count the number of tweets in each month of 2016 
# Call this new data frame "monthly_counts" 
monthly_counts <- trump_tweets %>%
  filter(year == 2016) %>%
  group_by(month) %>%
  count() # tells the numbers in each row/how many times he tweeted

# 6. Plot the data as a line graph with month on the x-axis and number of tweets on the y-axis
ggplot(monthly_counts, aes(x = month, y = n, group = 1)) +
  geom_line() +
  labs(title = "Number of Trump tweets by month (2016)",
       y = "# of tweets")


## Exercise 2: Which month/year has the shortest intervals between tweets?

# 1. Use a pipe to calculate the interval and duration (in minutes) between tweets
# Start by creating a new "lag" column that has the date/time of the previous tweet
# Then create an "interval" column that stores the interval data between the two dates
# Finally, create an "interval_minutes" column which transforms that duration into minute units
# ****Make sure your data is arranged in ascending order, starting with the 
# oldest date, before creating the lag!

trump_tweets <- trump_tweets %>%
  arrange(created_at) %>%
  mutate(previous_tweet = lag(created_at),
         interval = previous_tweet %--% created_at,
         interval_minutes = time_length(interval, unit = "minutes"))

# 3. Use a pipe to find the tweet with the shortest amount of time, on average, between tweets
# ****Group the data by year and month, use the summarize command to calculate the
# average time between tweets (in minutes), and then present the data in ascending order

av_durations <- trump_tweets %>%
  group_by(year, month) %>% 
  summarize(average = mean(interval_minutes, na.rm = TRUE)) %>%
  arrange(average)


############# STRINGS TUTORIAL ################

# stringr cheatsheet:
# https://evoldyn.gitlab.io/evomics-2018/ref-sheets/R_strings.pdf

# Most commands take at least two inputs
# The first input is always the character string or vector of strings

# str_length() will return the length of a string
str_length("Leilah")

# str_sub() will return certain letters of a string based on their position
str_sub("Leilah", 1, 1)
str_sub("Leilah", 1, 2)
str_sub("Leilah", 2, 4)
str_sub("Leilah", -1, -1)

# str_sub() can also be used to replace certain characters in a string 
# The character string must be saved as an object first
name <- "Leilah"
str_sub(name, 1, 1) <- "X" # find the first through the first letter in Leilah and replace it with X
name

# str_to_upper(), str_to_lower(), and str_to_title() change cases
strings <- c("This is a sentence.", "This is also a sentence.")

str_to_lower(strings)
str_to_upper(strings)
str_to_title(strings)

# We can extract words with word() based on position
word(strings, 1, 3)

# str_count() will count the number of times some pattern occurs
# By counting the number of spaces we can count the number of words
str_count(strings, pattern = " ")

# We can split strings into their component words
str_split(strings, pattern = " ")
words <- str_split(strings, pattern = " ", simplify = TRUE)
words

# We can use the base R function table() to count how many times each word appears
table(words)

# stringr provides many commands related to pattern matching
emails <- c("leilah@yale.edu", "leilah@gmail.com", "leilah@aol.com", "leilah@harvard.edu")

# We to detect, locate, extract, replace, and split strings
# Each function takes two arguments: a vector of strings, and a pattern to match

# str_detect() detects presence or absence of a pattern and returns T/F
# str_subset() returns the elements that contain that pattern
str_detect(emails, pattern = ".edu")
s <- str_subset(emails, pattern = ".edu") # subsets the data to the characters we are looking for 

# str_count() tallies the number of matches in each element
str_count(emails, pattern = ".edu")

# We can find the number of times a string occurs in a vector using sum()
sum(str_detect(emails, pattern = "edu"))
sum(str_count(emails, pattern = "edu"))

# str_locate() locates the first position of a pattern and returns a numeric matrix
# with columns start and end
# str_locate_all() locates all matches and returns a list of numeric matrices
str_locate(emails, pattern = "edu")

starts <- str_locate(emails, pattern = "edu")[, 1]
starts 

# str_replace() and str_replace_all() will replace a matched pattern
# str_replace() will replace only the first instance of a pattern within each element;
# in this case, we get the same result using both because "edu" never appears more than
# once in a character string
str_replace(emails, pattern = "edu", "XXX")
str_replace_all(emails, pattern = "edu", "XXX")


## Exercise 1: How many tweets contained the word "Hillary" appear during each month of 2016?

# 1. Create a new column called "contains_Hillary" that indicates whether a tweet contained "Hillary"
# But first make sure to make all tweets lower case!

trump_tweets <- trump_tweets %>%
  mutate(text = str_to_lower(text), # make sizing consistent
         contains_Hillary = str_detect(text, "hillary"))

# 2. Use a pipe to only look at data from 2016. Then summarize the data to calculate the number of times
# the tweets containing "Hillary" for each month of 2016. Call this new data frame "Hillary_counts"
# show it by percent

Hillary_counts <- trump_tweets %>%
  filter(year == 2016) %>%
  group_by(month) %>%
  summarize(percent_tweets = sum(contains_Hillary)/n())

# 3. Plot the data as a line graph with month on the x-axis and number of tweets on the y-axis

ggplot(Hillary_counts, aes(x = month, y = percent_tweets, group = 1)) +
  geom_line() +
  labs(title = "Number of Trump tweets mentioning 'Hillary' by month (2016)",
       y = "# of tweets")



